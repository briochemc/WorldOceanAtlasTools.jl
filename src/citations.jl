cite_WOA(year, vv) = @match my_varname(vv) begin
    "DSi" || "DIP" || "DIN" => cite_WOA_Nutrients(year)
    "Temp" => cite_WOA_Temperature(year)
    "Salt" => cite_WOA_Salinity(year)
    "O2" || "O2sat" || "AOU" => cite_WOA_Oxygen(year)
    "Dens" || "Cond" => cite_WOD(year)
    _ => error("Not sure what you are trying to cite.")
end

cite_WOD(year) = @match my_year(year) begin
    ...
    _ => "I could not find a citation for WOD$(my_year(year)). Add it if you know where to find it!"
end

cite_WOA_Temperature(year) = @match my_year(year) begin
    ...
    _ => error("No citation for WOA$(my_year(year)) Temperature. Add it if it should be there!")
end

cite_WOA_Salinity(year) = @match my_year(year) begin
    ...
    _ => error("No citation for WOA$(my_year(year)) Salinity. Add it if it should be there!")
end

cite_WOA_Oxygen(year) = @match my_year(year) begin
    ...
    _ => error("No citation for WOA$(my_year(year)) AOU. Add it if it should be there!")
end

cite_WOA_Nutrients(year) = @match my_year(year) begin
    ...
    _ => error("No citation for WOA$(my_year(year)) Nutrients. Add it if it should be there!")
end
